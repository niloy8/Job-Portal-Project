...
const Alljobs = () => {
    ...
    return (
        <div>
            ...
            <div className="w-11/12 p-5  rounded-2xl m-5 text-center flex items-center gap-5">
                <button onClick={() => setSort(!sort)} className={`btn btn-info ${sort && 'btn-success'}`} >{sort === true ? "Sorted By Salary" : "Sort By salary"}</button>

                <input onKeyUp={(e) => setSearch(e.target.value)} type="text" className="input w-1/2 outline-1 outline-amber-300" />
            </div>
